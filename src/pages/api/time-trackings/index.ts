...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getTimeTrackingsPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'time_tracking');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.time_tracking.count(countOptions as unknown),
      prisma.time_tracking.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
